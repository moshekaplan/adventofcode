...
# big-picture states
STATE_DATA = 0
STATE_CREATE_MARKER = 1
STATE_READ_MARKER_DATA = 2
...
def parse_marker(data):
    m = re.search('(\d+)x(\d+)', data)
    marker = {'length':int(m.group(1)), 'repeat':int(m.group(2))}
    return marker

def decompress(data):
    # Overall state and output
    state = STATE_DATA
    output = []
    # Data specific for parsing markers
    marker_data = []
    marker_length = 0
    marker_repeat_cnt = None
    marker_repeat_data = []
    # Decompress the stream
    for char in data:
        if state == STATE_DATA:
            if char == MARKER_BEGIN:
                state = STATE_CREATE_MARKER
            else:
                output.append(char)
        elif state == STATE_CREATE_MARKER:
            if char == MARKER_END:
                state = STATE_READ_MARKER_DATA
                marker = parse_marker("".join(marker_data))
                marker_data = []
                marker_length = marker['length']
                marker_repeat_cnt = marker['repeat']
            else:
                marker_data.append(char)
        elif state == STATE_READ_MARKER_DATA:
            marker_repeat_data.append(char)
            marker_length -= 1
            if marker_length == 0:
                repeated = "".join(marker_repeat_data)
                marker_repeat_data = []
                output.append(repeated*marker_repeat_cnt)
                state = STATE_DATA
    return "".join(output)
            
def decompress_phase2(data, generate_string=False):
    # Overall state and output
    state = STATE_DATA
    output = []
    output_length = 0
    ...
    for char in data:
        if state == STATE_DATA:
            ...
            if char == MARKER_END:
                ...
                marker = parse_marker("".join(marker_data))
                marker_data = []
                marker_data_remaining = marker['length']
                marker_repeat_cnt = marker['repeat']
            else:
                marker_data.append(char)
        elif state == STATE_READ_MARKER_DATA:
            marker_repeat_data.append(char)
            marker_data_remaining -= 1
            if marker_data_remaining == 0:
                #print "Multiply", marker_repeat_cnt, "by", "".join(marker_repeat_data)
                data, length = decompress_phase2("".join(marker_repeat_data))
                marker_repeat_data = []
                #print "Result: ", marker_repeat_cnt * decompressed
                output_length += marker_repeat_cnt * length
                if generate_string:
                    output.append(marker_repeat_cnt * data)
                state = STATE_DATA
    return "".join(output), output_length

def test_phase1():
    tests = ["ADVENT", "A(1x5)BC", "(3x3)XYZ", "A(2x2)BCD(2x2)EFG", "(6x1)(1x3)A", "X(8x2)(3x3)ABCY"]
    for test in tests:
        print test
        decompressed = decompress(test)
        print len(decompressed)
        print decompressed
        print

def test_phase2():
    tests = ["(3x3)XYZ", "X(8x2)(3x3)ABCY", "(27x12)(20x12)(13x14)(7x10)(1x12)A", "(25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN"]
    for test in tests:
        #test = "(25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN"
        print test
        decompressed = decompress_phase2(test, False)
        print len(decompressed)
        print decompressed
        print
...
def phase2():
    decompressed = decompress_phase2(data)
    print decompressed
