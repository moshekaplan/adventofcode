...
def split_ipv7(data):
    ...
    for part in parts:
        if ']' in part:
            #print part
            hyper, address = part.split(']')
            hypernets.append(hyper)
            addresses.append(address)
        else:
            addresses.append(part)
    #print "MATCHES ", addresses, hypernets
    return addresses, hypernets

def contains_symmetric(part):
    length = len(part)
    for i in range(length-4+1):
        left = part[i:i+2]
        right = part[i+2:i+2+2][::-1]
        # Must have at least two different letters, twice
        if len(left) < 2 or len(set(left)) == 1:
            continue
...
def supports_TLS(data):
    addresses, hypernets = split_ipv7(data)
    if any(contains_symmetric(hyper) for hyper in hypernets):
        return False
    if any(contains_symmetric(address) for address in addresses):
        return True
    return False
...
def supports_SSL(data):
    addresses, hypernets = split_ipv7(data)
    abas = get_sequences(addresses)
    babs = get_sequences(hypernets)
    
    #print abas, babs
    babs_transformed = set([ bab_to_aba(bab) for bab in babs ])
    if set(abas) & set(babs_transformed):
        return True
    else:
        return False
